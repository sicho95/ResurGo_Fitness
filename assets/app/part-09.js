  ...
  function bind(){
    ...
    on("createProfile",createProfile); on("startToday",()=>start(plan()[0]?.id)); on("shortWeek",()=>{const p=profile(); if(p){makeWeek(p.id,true); save("Semaine minimale viable générée.").then(render);}}); on("regenShort",()=>{const p=profile(); if(p){makeWeek(p.id,true); save("Semaine minimale viable générée.").then(render);}}); on("regenNormal",()=>{const p=profile(); if(p){makeWeek(p.id,false); save("Semaine normale générée.").then(render);}}); on("activeRest",()=>{const p=profile(); if(p){state.activities.push({id:uid("activity"),profileId:p.id,source:"manual",type:"walk",startedAt:new Date().toISOString(),durationSeconds:1200}); save("Repos actif enregistré.").then(render);}});
    on("installPwa",installPwa); on("dismissPwa",()=>{state.settings.hideInstallPrompt=true; save().then(render);});
    $$("[data-rate-target]").forEach(b=>b.onclick=()=>{const input=el(b.dataset.rateTarget), val=Number(b.dataset.rateValue), m=mood[val]; if(!input||!m) return; input.value=val; const field=b.closest(".smileyField"), summary=field?.querySelector("summary"), details=field?.querySelector("details"); field?.classList.remove("empty","bad","warn","ok","good","great"); field?.classList.add(m.tone); field?.querySelectorAll("[data-rate-target]").forEach(x=>x.classList.toggle("selected",x===b)); if(summary) summary.innerHTML=`<b>${m.face}</b><small>${val}/5</small>`; if(details) details.open=false; if(["quickBack","quickKnee","quickFatigue"].includes(b.dataset.rateTarget)) saveQuickCheck(true); if(["backPain","kneePain","tendonPain","fatigue"].includes(b.dataset.rateTarget)) saveProfileFields();});
  ...
  async function init(){ registerPwaEvents(); state=(await get("app"))||clone(empty); state={...clone(empty),...state,settings:{...empty.settings,...(state.settings||{})},ui:{...empty.ui,...(state.ui||{})}}; state.ui.message=""; await loadBodyMaps(); render(); registerServiceWorker(); window.addEventListener("online",checkOnlineVersion); }
  init();
})();
