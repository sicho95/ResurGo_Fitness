  ...
  function bind(){
    ...
    $$("[data-week-window]").forEach(b=>b.onclick=()=>{const p=profile(); if(!p) return; p.availabilityDays=Number(b.dataset.weekWindow)===7?7:5; saveProfileFields({regen:true,msg:`Plan recalculé sur ${p.availabilityDays} jours.`});});
    ...
    ["profileName","gender","age","heightCm","startWeightKg","targetWeightKg","targetMonths","sportsHistory"].forEach(id=>{const x=el(id); if(x) x.onchange=()=>saveProfileFields();});
